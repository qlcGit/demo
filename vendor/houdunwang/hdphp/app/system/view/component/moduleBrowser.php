            ...
            <tr>
                ...
                <td class="text-right">
                    <if value="in_array($d['mid'],$useModules)">
                        <button class="btn btn-primary" title="{{$d['title']}}" mid="{{$d['mid']}}" onclick="selectModuleItem(this)">选择</button>
                        <else/>
                        <button class="btn btn-default" title="{{$d['title']}}" mid="{{$d['mid']}}" onclick="selectModuleItem(this)">选择</button>
                    </if>
                </td>
            </tr>
        </foreach>
...
<script>
    ...
    function confirmModuleSelectHandler() {
        ...
        $('.selectModulesBox .btn-primary').each(function () {
            ...
            modules.push({title: title, mid: mid});
        })
